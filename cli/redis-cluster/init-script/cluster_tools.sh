...
get_rest_master_id_list_comma() {
    INITIAL_NODES=$(construct_initial_node_set "$REPLICAS" "$SERVICE_NAME" "$NAMESPACE" "$POD_NAME")
    CLUSTER_NODE_DNS=$(get_healthy_cluster_node_dsn "$INITIAL_NODES")
    MYID=$(get_my_node_id)  # ← Ensure your current node ID is available
...
move_all_slots_to_masters() {
    ...
    while [ "$ATTEMPT" -le "$MAX_RETRIES" ]; do
        ...
        CLUSTER_TO_ARGS=$(get_rest_master_id_list | awk '{printf "--cluster-to %s ", $0}')        
        
        if redis-cli --cluster reshard "$CLUSTER_NODE_DNS:$PORT" --cluster-from "$CURRENT_MASTER_ID" $CLUSTER_TO_ARGS --cluster-slots 16500 --cluster-yes; then
